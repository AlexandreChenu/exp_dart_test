...
namespace robot_dart {
	...
	while ((_world->getTime() - old_t - max_duration) < -factor && !_graphics->done()) {
            for (auto& robot : _robots)
                robot->update_nn(_world->getTime());
    ...
    void RobotDARTSimu::run_nn(double max_duration, Model model, Eigen::Vector3d target)
    {
        _break = false;
        size_t index = _old_index;
        double old_t = _world->getTime();
    ...
    while ((_world->getTime() - old_t - max_duration) < -factor && !_graphics->done()) {
            for (auto& robot : _robots)
                robot->update_nn(_world->getTime(), model, target); //only one robot in our case? 
